...
int egg_client_init() {
  ...
  if (g.bits==8) {
    ...
    if (egg_texture_load_image(g.texid_tiles=egg_texture_new(),RID_image_tiles16)<0) return -1;
    if (egg_texture_load_image(g.texid_sprites=egg_texture_new(),RID_image_sprites16)<0) return -1;
  }
